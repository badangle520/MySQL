...
SELECT e.employee_id,e.last_name,e.salary,avg_dep.*
FROM
    (
        SELECT
            department_id,ROUND(AVG(salary),0)平均工资
        FROM
            employees e
        GROUP BY department_id
    ) avg_dep INNER JOIN employees e ON avg_dep.department_id=e.department_id
WHERE e.salary>avg_dep.平均工资;
...
SELECT e.job_id
FROM employees e
WHERE e.department_id IN (SELECT DISTINCT department_id FROM departments WHERE location_id=1700);
